...
<head>
    ...
    <meta name="description" content="Professional Binary Trading Analyzer - Real-time signals with offline support">
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .online-badge { background: #00C85320; border: 1px solid #00C853; }
        input, select { background: #0B0F1A; border: 1px solid #2D5AFF40; border-radius: 12px; padding: 10px 14px; color: white; }
...
<div class="max-w-7xl mx-auto px-3 sm:px-5 py-4 pb-24">
    ...
    <div class="flex justify-between items-center mb-6 flex-wrap gap-2">
        <div>
            ...
            <p class="text-gray-400 text-xs mt-1">Real-time | PWA | Offline Support</p>
        </div>
        <div class="flex gap-2">
            <div id="onlineStatus" class="online-badge rounded-xl px-3 py-2 text-center">
                <div class="text-xs text-green-400"><i class="fas fa-wifi"></i> Online</div>
            </div>
            <div class="live-price-card rounded-xl px-3 py-2 text-center">
                <div class="text-xs text-gray-400">EUR/USD</div>
                <div class="text-lg font-bold text-green-400" id="livePrice">1.0925</div>
                <div class="text-[10px]" id="priceChange">▲ 0.00%</div>
            </div>
            <button onclick="showSettings()" class="p-2 rounded-xl bg-[#121826] hover:bg-[#1a2035]"><i class="fas fa-sliders-h text-blue-400"></i></button>
            <button onclick="exportToCSV()" class="p-2 rounded-xl bg-[#121826] hover:bg-[#1a2035]"><i class="fas fa-download text-green-400"></i></button>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <div class="card p-3 card-hover"><div class="text-gray-400 text-xs">Total Capital</div><div class="text-xl font-bold" id="totalCapital">$0</div></div>
        <div class="card p-3 card-hover"><div class="text-gray-400 text-xs">Available</div><div class="text-xl font-bold text-green-400" id="availableBalance">$0</div></div>
        <div class="card p-3 card-hover"><div class="text-gray-400 text-xs">Today's P/L</div><div class="text-xl font-bold" id="todayPL">$0</div></div>
        <div class="card p-3 card-hover"><div class="text-gray-400 text-xs">Win Rate</div><div class="text-xl font-bold text-blue-400" id="winRate">0%</div></div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-3 mb-5">
        ...
        <div class="card p-3">
            ...
            <div class="w-full bg-gray-700 rounded-full h-2 mt-2"><div id="riskBar" class="bg-green-500 h-2 rounded-full" style="width: 0%"></div></div>
    ...
    <div class="flex gap-1 mb-5 bg-[#121826] p-1 rounded-2xl sticky top-2 z-10 glass-effect overflow-x-auto">
        <button onclick="switchTab('trade')" id="tabTrade" class="tab-btn flex-1 py-2 rounded-xl text-sm font-medium bg-blue-600 text-white">➕ Trade</button>
        <button onclick="switchTab('journal')" id="tabJournal" class="tab-btn flex-1 py-2 rounded-xl text-sm font-medium bg-[#121826] text-gray-400">📅 Journal</button>
        <button onclick="switchTab('analytics')" id="tabAnalytics" class="tab-btn flex-1 py-2 rounded-xl text-sm font-medium bg-[#121826] text-gray-400">📈 Analytics</button>
        <button onclick="switchTab('patterns')" id="tabPatterns" class="tab-btn flex-1 py-2 rounded-xl text-sm font-medium bg-[#121826] text-gray-400">🧠 Patterns</button>
        <button onclick="switchTab('builder')" id="tabBuilder" class="tab-btn flex-1 py-2 rounded-xl text-sm font-medium bg-[#121826] text-gray-400">⚙️ Builder</button>
    </div>
    ...
    <div id="tradeSection" class="space-y-4">
        <div class="card p-4">
            ...
            <div class="grid grid-cols-2 gap-3 mb-3">
                <select id="signalTrend"><option value="Uptrend">📈 Uptrend</option><option value="Downtrend">📉 Downtrend</option><option value="Sideways">➡️ Sideways</option></select>
                <select id="signalZone"><option value="Support">🛡️ Support</option><option value="Resistance">⚡ Resistance</option><option value="Neutral">🌊 Neutral</option></select>
                <select id="signalPattern"><option value="Engulfing">🕯️ Engulfing</option><option value="Pin Bar">📌 Pin Bar</option><option value="Doji">⏸️ Doji</option><option value="Breakout">🚀 Breakout</option></select>
                <select id="signalSession"><option value="London">🇬🇧 London</option><option value="NY">🇺🇸 NY</option><option value="Asia">🇯🇵 Asia</option><option value="Overlap">🔥 Overlap</option></select>
            </div>
        ...
        <div class="card p-4">
            ...
            <div class="grid grid-cols-2 gap-3">
                <select id="tradePair"><option value="EUR/USD">EUR/USD</option><option value="GBP/USD">GBP/USD</option><option value="USD/JPY">USD/JPY</option><option value="AUD/USD">AUD/USD</option></select>
                <select id="tradeSession"><option value="London">🇬🇧 London</option><option value="NY">🇺🇸 NY</option><option value="Asia">🇯🇵 Asia</option><option value="Overlap">🔥 Overlap</option></select>
                <select id="tradeType"><option value="CALL">📈 CALL</option><option value="PUT">📉 PUT</option></select>
                <select id="tradePattern"><option value="Engulfing">Engulfing</option><option value="Pin Bar">Pin Bar</option><option value="Doji">Doji</option><option value="Breakout">Breakout</option></select>
                <select id="tradeTrend"><option value="Uptrend">Uptrend</option><option value="Downtrend">Downtrend</option><option value="Sideways">Sideways</option></select>
                <select id="tradeZone"><option value="Support">Support</option><option value="Resistance">Resistance</option><option value="Neutral">Neutral</option></select>
                <select id="tradeExpiry"><option value="1m">1 Minute</option><option value="2m">2 Minutes</option><option value="5m">5 Minutes</option></select>
                <input type="number" id="tradeConfidence" placeholder="Confidence % (0-100)" value="70">
                <input type="number" id="tradeAmount" placeholder="Amount ($)" class="col-span-2">
                <select id="tradeResult"><option value="WIN">✅ WIN</option><option value="LOSS">❌ LOSS</option></select>
    ...
    <div id="journalSection" class="hidden space-y-4">
        <div class="card p-4">
            <div class="flex justify-between items-center mb-3"><h3 class="font-bold">📅 Daily Journal</h3><button onclick="resetTodayConfirmation()" class="text-xs text-red-400"><i class="fas fa-trash"></i> Reset Today</button></div>
...
function connectWebSocket() {
    ...
    try {
        ...
        ws.onopen = () => {
            ...
            document.getElementById('onlineStatus').innerHTML = '<div class="text-xs text-green-400"><i class="fas fa-wifi"></i> Live</div>';
        };
        ...
        ws.onmessage = (event) => {
            ...
            document.getElementById('priceChange').className = `text-[10px] ${change >= 0 ? 'text-green-400' : 'text-red-400'}`;
        ...
        ws.onerror = () => {
            document.getElementById('onlineStatus').innerHTML = '<div class="text-xs text-yellow-400"><i class="fas fa-exclamation-triangle"></i> Delayed</div>';
            startRESTFallback();
        };
    } catch(e) {
        startRESTFallback();
...
function startRESTFallback() {
    ...
    autoRefreshInterval = setInterval(async () => {
        try {
            ...
            document.getElementById('livePrice').innerText = (1.0925 + (Math.random() - 0.5) * 0.001).toFixed(5);
        }
    }, 3000);
}
...
function detectPattern(prices) {
    ...
    if (last > prev && prev < prev2) return 'Engulfing';
    if (last < prev && prev > prev2) return 'Engulfing';
    if (Math.abs(last - prev) / prev < 0.0003) return 'Doji';
    if (last > prev * 1.001) return 'Breakout';
    return 'Pin Bar';
}
...
function autoAnalyzeMarket() {
    ...
    if (trend !== 'Sideways' && sr.zone !== 'Neutral') {
        ...
        let confidence = 65;
        if (pattern === 'Engulfing') confidence += 15;
        else if (pattern === 'Breakout') confidence += 10;
        else confidence += 5;
        if (sr.zone === 'Support' && signal === 'CALL') confidence += 10;
        if (sr.zone === 'Resistance' && signal === 'PUT') confidence += 10;
        ...
        if (signalDiv) {
            signalDiv.innerHTML = `
                <div class="p-3 bg-green-500/20 border border-green-500 rounded-xl animate-pulse">
                    ...
                    <div class="text-sm">Pattern: ${pattern} | Trend: ${trend} | Zone: ${sr.zone}</div>
                    <div class="text-xs text-yellow-300">Confidence: ${confidence}% ⚡ Real-time</div>
                </div>
            `;
            if (confidence >= 75) sendPushNotification('🎯 Auto Signal', `${signal} with ${confidence}% confidence`);
        }
    }
}
...
function addTrade() {
    ...
    if (result === 'WIN' && pl > 100) sendPushNotification('Big Win! 🎉', `${appState.currency}${pl.toFixed(2)} on ${pair}`);
    else if (result === 'LOSS' && pl < -100) sendPushNotification('Significant Loss ⚠️', `${appState.currency}${Math.abs(pl).toFixed(2)} on ${pair}`);
}
...
function generateDemoTrades() {
    ...
    for (let i = 0; i < 25; i++) {
        ...
        demo.push({
            id: Date.now() + i, date: date.toISOString().split('T')[0],
            ...
            result: Math.random() > 0.45 ? 'WIN' : 'LOSS', pl: 0
        });
    }
...
function saveData() {
    localStorage.setItem('tradeAnalyzerData', JSON.stringify({
        capital: appState.capital, riskMode: appState.riskMode,
        currency: appState.currency, trades: appState.trades
    }));
}
...
function generateManualSignal() {
    ...
    else if (zone === 'Resistance') { confidence += 20; reasons.push('⚡ Resistance'); if (trend === 'Downtrend') confidence +=
